...
# GUI class requires many widgets and methods for different tabs
# Widgets are created in setup methods called from __init__
class SimtempGUI:
    """GUI application for NXP Simtemp temperature monitoring."""
    ...
    def __init__(self, root):
        ...
        self.alpha = 0.3  # Exponential smoothing (lower = smoother)

        # Dynamic timing parameters
        self.start_timestamp = None  # First timestamp for relative time
        self.current_sampling_ms = None  # Current driver sampling
    ...
    def setup_monitor_tab(self, parent):
        ...
        values_frame = ttk.LabelFrame(
            parent, text="📊 Current Values", padding=10)
        ...
        ttk.Label(
            ...
            padx=10,
            pady=5)
        self.temp_display = ttk.Label(
            values_frame, textvariable=self.current_temp, font=(
        ...
        ttk.Label(
            ...
            padx=10,
            pady=5)
        self.alert_display = ttk.Label(
            values_frame, textvariable=self.current_alert, font=(
                "Arial", 12, "bold"), foreground="#27ae60")
        ...
        self.fig.canvas.mpl_connect(
            'draw_event', lambda event: None)  # Reduce overhead
    ...
    def setup_config_tab(self, parent):
        ...
        ttk.Checkbutton(
            ...
            command=self.toggle_device).pack(
            anchor=tk.W,
            padx=5,
            pady=2)
        ...
        ttk.Label(
            sampling_frame,
            text="Sampling Period (ms):").grid(
            row=0,
            column=0,
            sticky=tk.W,
            padx=5,
            pady=2)
        sampling_entry = ttk.Entry(
            sampling_frame,
            textvariable=self.sampling_ms,
            width=10)
        sampling_entry.grid(row=0, column=1, sticky=tk.W, padx=5, pady=2)
        ttk.Button(
            sampling_frame,
            text="Set",
            command=self.set_sampling).grid(
            row=0,
            column=2,
            padx=5,
            pady=2)
        ...
        ttk.Button(
            actions_frame,
            text="Load Current Config",
            command=self.load_config).pack(
            side=tk.LEFT,
            padx=5,
            pady=5)
        ttk.Button(
    ...
    def setup_stats_tab(self, parent):
        ...
        ttk.Button(
            ...
            command=self.update_stats).pack(
            pady=5)
    ...
    def setup_plot(self):
        ...
        # Temperature line - reduced markers for smoother rendering
    ...
    def check_and_adapt_framerate(self):
        ...
        try:
            ...
            with open('/sys/class/misc/simtemp/sampling_ms',
            ...
            if abs(sampling_ms - self.current_sampling_ms) > 2:
                ...
                if sampling_ms <= 10:
                    ...
                    # Slower sampling: update at 2x rate for interpolation
                    # smoothness
                ...
                print(
                    f"🎯 Adapted: sampling={sampling_ms}ms → update={
                        self.adaptive_interval}ms ({
                        1000 // self.adaptive_interval} FPS)")
    ...
    # Animation function requires comprehensive data processing
    def update_plot(self, frame):  # pylint: disable=unused-argument
        ...
        try:
            ...
            # Use the first recorded timestamp as reference, not the oldest in
            # the deque
            ...
            plot_times = [(ts - start_time).total_seconds()
            ...
            if len(temperatures_copy) >= 4:
                ...
                try:
                    ...
                    t_smooth = np.linspace(
                        t_array.min(), t_array.max(), num_points)

                    # B-spline for smooth curve (k=3 for cubic)
                    spl = make_interp_spline(
                        t_array, temp_array, k=min(
                            3, len(t_array) - 1))
            ...
            # Only update axes if limits changed significantly (reduces
            # flicker)
            ...
            if self.update_counter % 4 == 0:
                ...
                self.ax.xaxis.set_major_formatter(
                    plt.FuncFormatter(lambda x, p: f'{int(x)}s'))
            ...
            # Handle plot update errors to prevent crashes
    ...
    def get_optimal_update_interval(self):
        ...
        try:
            ...
            # If sampling is slow (>50ms), match it exactly for 1:1
            # correspondence
            ...
            if sampling_ms <= 33:
                ...
                # Medium sampling: use sampling rate or 30 FPS, whichever is
                # faster
            ...
            print(
                f"📊 Sampling period: {sampling_ms}ms → GUI update: {interval}ms ({
                    1000 // interval} FPS)")
            return interval

        except (OSError, ValueError, AttributeError) as e:
            print(
                f"Warning: Could not read sampling period ({e}), using 33ms default")
            return 33  # Default to 30 FPS

    def start_monitoring(self):
        """Start temperature monitoring"""
        if not os.path.exists(self.device.device_path):
            msg = (f"Device {self.device.device_path} not found.\n"
        ...
        if hasattr(
                self,
                'anim') and optimal_interval != self.gui_update_interval:
        ...
        while self.monitoring:
            try:
                ...
                if flags & SIMTEMP_FLAG_THRESHOLD_CROSSED:
                    ...
                    self.root.after(
                        0, lambda: self.alert_display.config(
                            foreground="#e74c3c"))
                ...
                self.alerts.append(
                    bool(flags & SIMTEMP_FLAG_THRESHOLD_CROSSED))
        ...
        try:
            ...
            if period < 10 or period > 5000:
                messagebox.showerror(
                    "Error", "Sampling period must be between 10 and 5000 ms")
                return

            if self.device.set_sampling_period(period):
                messagebox.showinfo(
                    "Success", f"Sampling period set to {period} ms")
    ...
    def set_threshold(self):
        ...
        try:
            ...
            if threshold < -50 or threshold > 150:
                messagebox.showerror(
                    "Error", "Threshold must be between -50°C and 150°C")
                return

            threshold_mc = int(threshold * 1000)
            if self.device.set_threshold(threshold_mc):
                messagebox.showinfo(
                    "Success", f"Threshold set to {
                        threshold:.1f}°C")
            else:
    ...
    def test_alert(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Test Alert",
                "Threshold set to 25°C. Monitor for alert indication.")

            # Restore after a few seconds
            self.root.after(
                5000,
                lambda: [
                    self.threshold_c.set(original_threshold),
                    self.set_threshold()])
